...
export async function CreateUser(fullname, email, username, password) {
  try {
    const result = await executeQuery({
      query:
        "INSERT INTO userdata (fullname, username, email, userpassword) VALUES (?, ?, ?, ?)",
...
export async function CheckIfUser(email) {
  try {
    const connection = await databaseConnection();
    const [results] = await connection.query(`
        SELECT * FROM userdata WHERE email="${email}";
